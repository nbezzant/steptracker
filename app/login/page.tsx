  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative z-10 px-4">
      ...
      >
        <button
          onClick={async () => {
            ...
            try {
              ...
              setAuthError(`Sign-in failed: ${code}${msg ? " — " + msg : ""}`);
              setSigningIn(false);
            }
          }}
          disabled={signingIn}
          className="group flex items-center gap-3 glass rounded-2xl px-8 py-4 hover:bg-white/[0.06] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {signingIn ? (
            <div className="w-5 h-5 border-2 border-white/40 border-t-white/80 rounded-full animate-spin flex-shrink-0" />
          ) : (
        ...
        {authError && (
          <div className="mt-4 glass rounded-xl px-4 py-3 text-xs font-mono text-red-400 max-w-sm text-center">
            {authError}
          </div>
        )}
      </div>
